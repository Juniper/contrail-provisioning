...
class DatabaseCommon(ContrailSetup):
    ...
    def fixup_cassandra_config_file(self, listen_ip, seed_list, data_dir,
        ...
        conf_file = os.path.join(conf_dir, self.cassandra.conf_dir)
    ...
    def fixup_cassandra_env_config(self):
        env_file = os.path.join(self.cassandra.conf_dir, self.cassandra.env_file)
        cnd = os.path.exists(env_file)
        if not cnd:
            raise RuntimeError('%s does not appear to be a cassandra source directory' % self.cassandra.conf_dir)
