...
class DatabaseSetup(ContrailSetup):
	...
	if self._args.kafka_broker_id is not None:
            ...
            #kafka not enabled
            if os.path.exists('/etc/contrail/supervisord_database_files/contrail-kafka-database.ini'):
                os.remove('/etc/contrail/supervisord_database_files/contrail-kafka-database.ini')
