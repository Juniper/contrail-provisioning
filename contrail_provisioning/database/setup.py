...
from contrail_provisioning.database.templates import create_user_template
...
class DatabaseSetup(ContrailSetup):
    ...
    def parse_args(self, args_str):
        ...
        parser.add_argument("--cassandra_name", help = "Cassandra user name if provided")
    ...
    def create_user(self):
        template_vals = {
                        '__cassandra_name__': self._args.cassandra_name,
                        '__cassandra_password__': self._args.cassandra_password,
                        }
        self._template_substitute_write(create_user_template.template,
                                        template_vals, self._temp_dir_name + '/create_user')
        local("sudo mv %s/create_user /etc/contrail/create_user" %(self._temp_dir_name))
        ...
        while connected == False and retry < retry_threshold:
            ...
            status = subprocess.Popen('sudo cqlsh %s  -u cassandra -p cassandra -f /etc/contrail/create_user' % self.database_listen_ip, shell=True,stderr = subprocess.PIPE,stdout=subprocess.PIPE).stderr.read()
