...
class DatabaseSetup(DatabaseCommon):
    ...
    def fixup_kafka_server_properties(self, listen_ip):
        ...
        self.replace_in_file(KAFKA_SERVER_PROPERTIES, 'log.cleaner.dedupe.buffer.size=.*',
                'log.cleaner.dedupe.buffer.size=1000000000')
        ...
        if not self.file_pattern_check(KAFKA_SERVER_PROPERTIES, 'log.cleaner.dedupe.buffer.size=1000000000',
            local('sudo echo "log.cleaner.dedupe.buffer.size=1000000000" >> %s' % KAFKA_SERVER_PROPERTIES)
