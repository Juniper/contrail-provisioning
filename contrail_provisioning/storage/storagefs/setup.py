...
class SetupCeph(object):
    ...
    MAX_POOL_COUNT = 1024
    # Ceph pool list will contain the names of all the pools
    # created during create_hdd_ssd_pool()
    # This will be later used during the cinder configuration
    # when creating the backends.
    ...
    # All the above are performed in a loop of pool count. The Pool count is
    # derived from the number of unique pool configured testbed.py.
    ...
    def __init__(self, args_str = None):
                    ...
                    for hostname, entries, entry_token in zip(self._args.storage_hostnames, self._args.storage_hosts, self._args.storage_host_tokens):
                        for disks in storage_disk_list:
                            ...
                            if disksplit[0] == add_storage_node and disksplit[0] == hostname:
                                ...
                                # if there are only 2 variable in the disk specification,
                                # check if the check the 2nd entry is the journal disk
                                # or the Pool name. Based on the above discard the Pool name
                                diskcount = disks.count(':')
                                if diskcount == 3:
        ...
        if configure_with_ceph == 1:
            ...
            # Based on the hdd/ssd pools created, apply the ceph auth information
            if create_hdd_ssd_pool == 1:
            ...
            if create_hdd_ssd_pool == 1:
                # Based on the hdd/ssd pools created, create and set the virsh secrets
                for pool_name in ceph_pool_list:
