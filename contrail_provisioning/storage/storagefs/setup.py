...
class SetupCeph(object):
    ...
    def restart_service_withtimeout(self, service_name, timeout=15, retries=3):
        while retries:
            try:
        ...
        if pdist == 'Ubuntu':
            ...
            for entries, entry_token in zip(self._args.storage_hosts, \
                ...
                if self._args.storage_master == entries:
                    with settings(host_string = 'root@%s' %(entries),
                        password = entry_token):
                        self.restart_service_withtimeout('cinder-api')
                        self.restart_service_withtimeout('cinder-scheduler')
                        self.restart_service_withtimeout('cinder-volume')
                        self.restart_service_withtimeout('glance-api')
                        self.restart_service_withtimeout('nova-api')
                        self.restart_service_withtimeout('nova-conductor')
                        self.restart_service_withtimeout('nova-scheduler')
                        self.restart_service_withtimeout('libvirt-bin')
            if self._args.storage_os_hosts[0] != 'none':
                for entries, entry_token in zip(self._args.storage_os_hosts,
                                            self._args.storage_os_host_tokens):
                    with settings(host_string = 'root@%s' %(entries),
