...
main() {
    [[ $argc -lt 3 ]] && { echo "Usage: $0 NODE_IP SSL_PATH CERT_FILE_PREFIX";
                           echo "Example: $0 10.1.1.100 /etc/contrail/ apiserver";
                           exit 1;
                         }
...
#Create Root and server pem files 
   ...
   cd ../
   $CP working/"$CERT_FILE_PREFIX".key $SSL_PATH
   $CP working/"$CERT_FILE_PREFIX".pem $SSL_PATH
   $CP working/"$CERT_FILE_PREFIX"_ca.pem $SSL_PATH
   $RM -rf working
}
