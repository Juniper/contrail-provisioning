...
class ComputeBaseSetup(ContrailSetup, ComputeNetworkSetup):
    def __init__(self, compute_args, args_str=None):
        ...
        self.contrail_openstack = not(getattr(self._args,
            'no_contrail_openstack', False))
        self.config_nova = not(getattr(self._args,
            'no_nova_config', False))
        if ('tsn' in self._args.roles or
            self._args.orchestrator == 'vcenter'):
            self.contrail_openstack = False
            self.config_nova = False
