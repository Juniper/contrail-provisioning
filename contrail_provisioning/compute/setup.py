...
class ComputeSetup(ContrailSetup):
    ...
    def parse_args(self, args_str):
        ...
        parser.add_argument("--openstack_mgmt_ip",
            help="Mgmt IP Address of the openstack node if it is different from openstack_IP")
        ...
        parser.add_argument("--non_mgmt_ip",
            help="IP Address of non-management interface(fabric network) on the compute  node")
        parser.add_argument("--non_mgmt_gw",
            help="Gateway Address of the non-management interface(fabric network) on the compute node")
        ...
        parser.add_argument("--vgw_public_vn_name",
            help="Fully-qualified domain name (FQDN) of the routing-instance that needs public access")
        parser.add_argument("--vgw_intf_list", help="List of virtual getway intreface")
        parser.add_argument("--vgw_gateway_routes",
            help="Static route to be configured in agent configuration for VGW")
        parser.add_argument("--public_vn_name",
            help="Fully-qualified domain name (FQDN) of the routing-instance that needs public access")
        parser.add_argument("--gateway_routes",
            help="List of route need to be added in agent configuration for virtual gateway")
        ...
        parser.add_argument("--no_contrail_openstack", action="store_true",
            help="Do not provision contrail Openstack in compute node.")
        parser.add_argument("--no_nova_config", action="store_true",
            help="Do not configure anything related to nova.")
        ...
        parser.add_argument("--cpu_mode",
            help="VM cpu_mode, [none|host-model|host-passthrough|custom]")
        parser.add_argument("--cpu_model",
            help="VM cpu_model, required if cpu_mode is 'custom'. eg. 'Nehalem'")
        ...
        parser.add_argument("-R", "--roles", nargs='+', type=str,
            help="List of contrail roles played by this node.")
