...
if [ $SRIOV_INTERFACES != "" ]; then
    ...
    openstack-config --set /etc/nova/nova-compute.conf DEFAULT scheduler_default_filters PciPassthroughFilter
    openstack-config --set /etc/nova/nova-compute.conf DEFAULT scheduler_available_filters nova.scheduler.filters.all_filters
