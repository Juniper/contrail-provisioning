...
from contrail_provisioning.config.templates import ifmap_ini
...
class ConfigBaseSetup(ContrailSetup):
    ...
    def fixup_ifmap_config_files(self):
        if self.pdist == 'Ubuntu' or self.pdist == 'centos' or self.pdist == 'redhat':
            ...
            # Disable/enable irond service
            if self._args.use_irond:
                self._template_substitute_write(ifmap_ini.template, {}, self._temp_dir_name + '/ifmap.ini')
                local("sudo mv %s/ifmap.ini /etc/contrail/supervisord_config_files/" % self._temp_dir_name)
            else:
                local("sudo rm -f /etc/contrail/supervisord_config_files/ifmap.ini")
    ...
    def fixup_contrail_api_config_file(self):
        ...
        if self._args.use_irond:
            ifmap_server_conf = {
                'ip': self.cfgm_ip,
                'port': '8444' if self._args.use_certs else '8443',
                'username': 'api-server',
                'password': 'api-server',
            }
            template_vals['__contrail_ifmap_server_conf__'] = """
ifmap_server_ip=%(ip)s
ifmap_server_port=%(port)s
ifmap_username=%(username)s
ifmap_password=%(password)s""" % ifmap_server_conf
            template_vals['__contrail_ifmap_embedded_server_conf__'] = ''
            if self._args.use_certs:
                ifmap_cert_conf = {
                    'key_file': '/etc/contrail/ssl/private_keys/schema_xfer_key.pem',
                    'cert_file': '/etc/contrail/ssl/certs/schema_xfer.pem',
                    'ca_file': '/etc/contrail/ssl/certs/ca.pem',
                }
                template_vals['__contrail_ifmap_cert__'] = """
[SECURITY]
use_certs=True
keyfile=%(key_file)s
certfile=%(cert_file)s
ca_certs=%(ca_file)s""" % ifmap_cert_conf
            else:
                template_vals['__contrail_ifmap_cert__'] = ''
        else:
            ifmap_embedded_server_conf = {
                'ip': self.cfgm_ip,
                'port': 8443,
                'creds': "%s %s" % (' '.join(self.control_node_users.split('\n')),
                                    ' '.join(self.control_node_dns_users.split('\n')))
            }
            template_vals['__contrail_ifmap_embedded_server_conf__'] = """
[IFMAP_SERVER]
# IF-MAP embedded server configuration
# Listen IP and port
ifmap_listen_ip = %(ip)s
ifmap_listen_port = %(port)s
# Key and certificate files path. If not set, automatically created
...
ifmap_credentials = %(creds)s""" % ifmap_embedded_server_conf
            ifmap_server_conf = {
                'ip': self.cfgm_ip,
            }
            # Keep the DEFUALTS.ifmap_server_ip as it is the config flag used
            # to publish ApiServer service to discovery.
            template_vals['__contrail_ifmap_server_conf__'] = """
ifmap_server_ip=%(ip)s""" % ifmap_server_conf
            template_vals['__contrail_ifmap_cert__'] = ''
