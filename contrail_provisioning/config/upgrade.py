...
class ConfigUpgrade(ContrailUpgrade, ConfigSetup):
    ...
    def update_upgrade_data(self):
        ...
        if self.pdist in ['centos', 'redhat']:
            if (self._args.from_rel < LooseVersion('2.0') and
                self._args.to_rel >= LooseVersion('2.2')):
                ifmap_dir = '/etc/irond'
        ...
        if (self._args.from_rel >= LooseVersion('2.1')):
    ...
    def upgrade(self):
        ...
        if (self._args.from_rel < LooseVersion('2.1') and
            self._args.to_rel >= LooseVersion('2.1')):
            self.config_setup.fixup_device_manager_config_file()
        # Seperate contrail-<role>-nodemgr.conf is introduced from release 2.20
        if (self._args.from_rel < LooseVersion('2.2') and
            self._args.to_rel >= LooseVersion('2.2')):
