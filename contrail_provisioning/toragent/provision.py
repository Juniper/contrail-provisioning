...
frontend http_server_portdiscovery *:5998
...
@task
def prov_tsn():
    """Provisions required config in all TSN node mentioned in testbed. USAGE: fab prov_tsn."""
    execute("prov_tsn_node", env.host_string)

@task
def prov_tsn_node(*args):
    """Provisions required config in particular  TSN node mentioned in testbed. USAGE: fab prov_tsn_node."""
    for host_string in args:
        with settings(host_string=host_string, warn_only=True):
            nova_conf_file = '/etc/contrail/contrail-vrouter-agent.conf'
            local("sudo sed -i 's/#tsn=false/tsn=true/g' %s" \
                   % (nova_conf_file))
...
@task
def prov_toragent():
    """Provisions required confiig for all TOR agent node. USAGE: fab prov_toragent."""
    execute("prov_toragent_node", env.host_string)

@task
def prov_toragent_node(*args):
    """Provisions required confiig for particular TOR agent node. USAGE: fab prov_toragent_node."""
    for host_string in args:
        with settings(host_string=host_string, warn_only=True):
            ...
            for i in range(len(toragent_dict[host_string])):
                ...
                agent_name= 'tor-agent-' + str(tor_id)
