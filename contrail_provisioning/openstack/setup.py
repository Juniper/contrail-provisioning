...
class OpenstackSetup(ContrailSetup):
    ...
    def build_ctrl_details(self):
        ...
        ctrl_infos.append('CONTROLLER=%s' % self._args.self_ip)
            ...
            if self._args.internal_vip:
                ...
                if os.path.exists(apache2_keystone_conf):
                    with settings(warn_only=True):
                        ...
                        local("sudo sed -i 's/<VirtualHost \*:5000>/<VirtualHost *:6000>/g' %s" % apache2_keystone_conf)
                        local("sudo sed -i 's/<VirtualHost \*:35357>/<VirtualHost *:35358>/g' %s" % apache2_keystone_conf)
                apache2_keystone_conf = "/etc/apache2/sites-available/keystone.conf"
                if os.path.exists(apache2_keystone_conf):
                    with settings(warn_only=True):
                        local("sudo sed -i 's/Listen 5000/Listen 6000/g' %s" % apache2_keystone_conf)
                        local("sudo sed -i 's/Listen 35357/Listen 35358/g' %s" % apache2_keystone_conf)
