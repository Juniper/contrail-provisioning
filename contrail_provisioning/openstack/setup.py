...
class OpenstackSetup(ContrailSetup):
    ...
    def unregister_all_services(self):
        hostname = sudo('getent hosts %s | awk \'{print $2}\'' % self._args.node_to_unregister)
        with prefix('source /etc/contrail/openstackrc'):
            service_list = sudo("nova service-list | grep %s | awk '{print $2}'" % hostname).split()

            for service in service_list:
                sudo('nova service-delete %s' % service)
