...
if [ "$INTERNAL_VIP" != "none" ]; then
    openstack-config --set /etc/barbican/barbican-api-paste.ini filter:keystone_authtoken identity_uri http://$INTERNAL_VIP:35358
else
    openstack-config --set /etc/barbican/barbican-api-paste.ini filter:keystone_authtoken identity_uri http://localhost:35357
fi

openstack-config --set /etc/barbican/barbican.conf DEFAULT bind_port 9113
...
if [ "$INTERNAL_VIP" != "none" ]; then
    openstack-config --set /etc/barbican/barbican.conf DEFAULT sql_connection mysql://barbican:$SERVICE_DBPASS@$CONTROLLER:3306/barbican
else
