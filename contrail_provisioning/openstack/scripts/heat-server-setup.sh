...
for svc in heat; do
    ...
    openstack-config --set /etc/$svc/$svc.conf keystone_authtoken auth_uri $AUTH_PROTOCOL://$controller_ip:5000/$SERVICE_TOKEN
...
update_services "action=restart" $openstack_services_heat
