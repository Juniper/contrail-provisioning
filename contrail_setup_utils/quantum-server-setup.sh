...
if [ -d /etc/neutron ]; then
    ...
    openstack-config --del /etc/neutron/neutron.conf service_providers service_provider LOADBALANCER:Haproxy:neutron.services.loadbalancer.drivers.haproxy.plugin_driver.HaproxyOnHostPluginDriver:default
    openstack-config --del /etc/neutron/neutron.conf service_providers service_provider VPN:openswan:neutron.services.vpn.service_drivers.ipsec.IPsecVPNDriver:default
...
if [ "$INTERNAL_VIP" != "none" ]; then
    ...
    openstack-config --set /etc/$net_svc_name/$net_svc_name.conf DEFAULT rabbit_port $AMQP_PORT
